...
func main() {
	var saveMode bool
	flag.BoolVar(&saveMode, "save", false, "save a pattern (e.g: xgf -save pat-name -Hnri 'search-pattern')")
...
func getPatternDir() (string, error) {
	...
	path := filepath.Join(usr.HomeDir, ".config/xgf")
	if _, err := os.Stat(path); !os.IsNotExist(err) {
		// .config/xgf exists
		return path, nil
	}
	return filepath.Join(usr.HomeDir, ".xgf"), nil
}
